...
object CRUD {
  ...
  // Swoją drogą niby wszystko dziedziczy po Control, ale to  jest zdefiniowane jako abstract... polimorfizm ograniczony, dla różnych kontrolek inaczej :(
  ...
  def elementsList(pageName : String, uri : String,
    ...
    val wnd : GridBagPanel = new GridBagPanel {
      ...
      siteData.foreach{row => {
        ...
        additionalCols.foreach{fun =>
          add(fun(row("id")), constraints(colId, rowId, fill=GridBagPanel.Fill.Both))
          colId+=1
        }
        rowId+=1
      }}
    ...
    // Wyczyszczenie, żeby nie zostało na kolejnej stronie
    additionalCols = List()
  }
  ...
  def cancelOwnVisit(id : String) = {
    new Button("Anuluj") {
      tooltip = "Anuluj swoją wizytę"
      listenTo(this)
  ...
  def cancelVisit(id : String) = new Button("Anuluj") {
      tooltip = "Anuluj wizytę pacjenta"
      listenTo(this)
  ...
  // Dodawanie komentarza do wizyty pacjenta
  def editVisit(id : String) = new Button("Edytuj") {
      tooltip = "Dodaj komentarz do tej wizyty"
      listenTo(this)
      reactions += {
        case ButtonClicked(_) => {
            CRUD.addEditAction("edytuj-wizyte-pacjenta-<id>" replaceAll("\\<id\\>", id), "Pacjenci zarejestrowani na dziś", Doctor.mainMenu, "Komentarz do wizyty pacjenta")
        }
      }
    }

  // Wyświetlanie historii leczenia pacjenta
  def patientHistory(id : String) = new Button("Historia") {
      tooltip = "Historia leczenia pacjenta"
      listenTo(this)
      reactions += { case ButtonClicked(_) => {
        repaintFunc(showHistory(id), "Historia leczenia pacjenta")
      }}
    }

  // Historia leczenia określonego pacjenta
  def showHistory(id : String) : BoxPanel = {
    val data = WebAPI.read("historia-leczenia-<id>" replaceAll("\\<id\\>", id))
    // Lista wyświetlanych kolumn
    val cols = List(Map("doctor" -> "Lekarz"), Map("date" -> "Data"), Map("comment" -> "Komentarz"))
    val content = new BoxPanel(Orientation.Vertical) {
      data.foreach{row => {
        cols.foreach{col => {
          contents += new BoxPanel(Orientation.Horizontal) { // Kolejne kolumny
            contents += new Label(col.head._2+": ") {
            ...
            contents += { // Wartość
              if(col.head._1 == "comment") new ScrollPane(new TextArea(row(col.head._1)) {editable=false}) {
                  preferredSize=new Dimension(700, 50)
                  maximumSize=preferredSize
                  minimumSize=preferredSize
                }
              else new Label(row(col.head._1)) {
                  preferredSize=new Dimension(700, 25)
                  maximumSize=preferredSize
                  minimumSize=preferredSize
                  horizontalAlignment=Alignment.Left
                  verticalAlignment=Alignment.Top
    ...
    new BoxPanel(Orientation.Vertical) {
      contents += new ScrollPane(content) {
        preferredSize = new Dimension(920, 300)
      }
      contents += VStrut(10)
      contents += new BorderPanel {
        add(new Button("Wróć do listy dzisiejszych wizyt") {
            listenTo(this)
            reactions += { case ButtonClicked(_) => CRUD.elementsList("Pacjenci zarejestrowani na dziś", "dzisiejsze-wizyty", cancelVisitColumns) }
          }, BorderPanel.Position.Center)
      }
    }
  }
