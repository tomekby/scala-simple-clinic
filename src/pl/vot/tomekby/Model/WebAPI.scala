...
object WebAPI {
  ...
  def login(username : String, pwd : String) : Boolean = {
    ...
    // Sprawdzenie grup
    isAdmin = checkInGroup("Administrator")
    isDoctor = checkInGroup("Lekarz")
    isReception = checkInGroup("Recepcja")
    loggedIn
  }
  ...
  private def checkInGroup(group : String) : Boolean = browser.get(strToUrl("api/group/"+group)).body.asString == "TRUE"
  ...
  def getMain() : String = browser.get(strToUrl("/")).body.asString
}
